...
function EditExam(props) {
  ...
  const beforeUpload=()=>{
    if(imageID!=="no image") axios.post(`https://localhost:5000/image/delete/${imageID}`)
    return true
  }
              ...
              <Form.Item name="dragger" valuePropName="fileList" getValueFromEvent={normFile} noStyle>
                <ImgCrop aspect={16 / 9} minZoom={0.5} quality={1} grid>
                  <Upload.Dragger onRemove={() => handleRemove(imageID)} onChange={onUploadChange} listType="picture" maxCount={1}
                    name="file" onPreview={onPreview} action='https://localhost:5000/image/upload' accept=".jpg, .jpeg, .png" beforeUpload={beforeUpload}>
