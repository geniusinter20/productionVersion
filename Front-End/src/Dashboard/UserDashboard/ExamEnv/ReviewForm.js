...
class QuestionsForm extends Component {
    ...
    handleFinish = () => {
        ...
        //     this.setState({answerwithid: { questionID: id, answer: ans[0] }})
    }
    renderQuestion=(question)=>{
        ...
        switch (question.qesType) {
            case "ordering":
                return(
                     <OrderingQuestion 
                  setAnswer={this.setAnswer} question={question} state={"review"}
    ...
    render() {
        ...
        //console.log("questions", this.props.len)
        const { examInfo, loadingData, loadingQuestions, currentQuestionNo } = this.state;
        const question = this.props.len[this.state.currentQuestionNo - 1] && !loadingQuestions?this.state.questions.filter(q=>q._id===this.props.len[this.state.currentQuestionNo - 1].questionID)[0]:null
        //console.log(this.props.len[this.state.currentQuestionNo - 1])
        ...
        return (
            <MainContainer>
                ...
                this.renderQuestion(this.state.questions[this.state.currentQuestionNo - 1])
                :null}
                <SubHeader>
                    <div style={{ color: "#444444", fontWeight: "600" }}>Explaination:</div>
                    {!loadingQuestions && <div>{this.state.questions[this.state.currentQuestionNo - 1].ansExp}</div>}
