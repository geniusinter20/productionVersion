...
export default function Profile() {
    ...
    const [imageUrl, setImageUrl]= useState(auth.userData.imageID?`http://exporagenius.com:5000/image/${auth.userData.imageID}`:profilePic)
    ...
    const onUploadChange = (event) => {
        ...
        if (event.file.response) {
            ...
            setImageUrl(`http://exporagenius.com:5000/image/${event.file.response.id}`)
            dispatch(updateImage(event.file.response.id))
            axios.post(`http://exporagenius.com:5000/client/updateimage/${auth.userData._id}`,{imageID:event.file.response.id})
        }
    }
                ...
                <Col xs={{ span: 24, offset: 0 }} lg={{ span: 4, offset: 0 }} style={{ position: "relative" }}>
                    ...
                    <ImgCrop aspect={1 / 1} minZoom={0.1} quality={1} grid>
                        <CusUpload showUploadList={false} onRemove={() => handleRemove(imageID)} onChange={onUploadChange} listType="picture" maxCount={1}
                            beforeUpload={beforeUpload} name="file" onPreview={onPreview} action='http://exporagenius.com:5000/image/upload' accept=".jpg, .jpeg, .png">
