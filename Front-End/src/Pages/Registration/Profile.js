...
export default function Profile() {
    ...
    const [imageUrl, setImageUrl]= useState(`http://92.205.62.248:5000/image/61d6e873a266163a5c944570`)
    const onFinish = () => {
        form.validateFields()
    ...
    const onUploadChange = (event) => {
        ...
        if (event.file.response) {
            ...
            setImageUrl(`http://92.205.62.248:5000/image/${event.file.response.id}`)
        }
    }
                ...
                <Col xs={{ span: 24, offset: 0 }} lg={{ span: 4, offset: 0 }} style={{ position: "relative" }}>
                    ...
                    <ImgCrop aspect={1 / 1} minZoom={0.1} quality={1} grid>
                        <CusUpload showUploadList={false} onRemove={() => handleRemove(imageID)} onChange={onUploadChange} listType="picture" maxCount={1}
                            beforeUpload={beforeUpload} name="file" onPreview={onPreview} action='http://92.205.62.248:5000/image/upload' accept=".jpg, .jpeg, .png">
