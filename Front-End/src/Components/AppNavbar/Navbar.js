...
import { Menu, Dropdown, Button, Affix, Tooltip } from 'antd';
...
function NavBar() {
  ...
  const menu = (
    <Menu>{
      onlineTrainingMenu.map((item, index) => (
        <MyMenueItem key={index} >
          <Link to={item.path} >
            {item.title}
          </Link>
        </MyMenueItem>
  ...
  const userMenu = (
    <UserMenu onClick={({ item, key, keyPath, domEvent }) => handleUserMenuClick(key)}>
      <UserMenueItem key="info">
        <MyCon>
          {userLogged ? auth.userData.imageID && auth.userData.imageID!=="no image" ? <Avatar src={`https://exporagenius.com:5000/image/${auth.userData.imageID}`} size={35}
          >

          </Avatar>
            : <MyAvatar style={{ borderStyle: "solid" }} size={35}
            >
              <div style={{ fontSize: 16 }}>{userInfo.fullName.charAt(0).toUpperCase()}</div>
            </MyAvatar> : <div></div>}
          <Typography sx={{ width: "190px", fontSize: "14px", fontWeight: "500" }} component="div" noWrap={true}>
  ...
  return (
    <div>
      <Affix onChange={affixed => setNavBarAffixed(affixed)}>
        <MyNavBar shadow={navBarAffixed ? "0px 2px 4px rgba(0, 0, 0, 0.16)" : ""} className="navbar1">
        
          <Link to="/" className="navbar-logo">
            <Tooltip placement="bottom" color="6c6c6c" title="Homepage">
          ...
          </ul>
          {
            !userLogged ? <ul className="Nav-batns">
              ...
              <Tooltip placement="bottom" color= "6c6c6c" title="Cart">
              ...
              <ul className="Nav-batns-logged">
                ...
                <li key="5" >
                  <Dropdown overlay={userMenu} placement="bottomRight">
                    {auth.userData.imageID && auth.userData.imageID!=="no image" ? <Avatar style={{ cursor: "pointer" }} src={`https://exporagenius.com:5000/image/${auth.userData.imageID}`} size={45}
                    >
      ...
      <div className="navbar2">
        ...
        >
          ...
          {userLogged ? <div style={{ marginRight: "20px", display: "flex", gap: "5px", alignItems: "center", cursor: "pointer" }}
            ...
            {
              auth.userData.imageID && auth.userData.imageID!=="no image" ? <Avatar src={`https://exporagenius.com:5000/image/${auth.userData.imageID}`} size={40}
              >
