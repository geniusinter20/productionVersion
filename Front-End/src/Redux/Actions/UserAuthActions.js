...
export const signUp = (user) => {
  ...
  return (dispatch) => {
    ...
    axios
      .post(`http://exporagenius.com:5000/client/register`, user)
      .then((token) => {
        if (token.data.msg) {
          dispatch({
            type: "SIGN_UP_FAIL",
...
export const signIn = (email, password, remembered) => {
  ...
  return (dispatch) => {
    ...
    axios.post(`http://exporagenius.com:5000/client/login`, { email, password })
      .then(({ data }) => {
        //console.log(data);
        if (data.msg) {
...
export const changePassword = (currentPassword, newPassword, email, userID) => {
  ...
  return (dispatch) => {
    let promiseA = axios.post(`http://exporagenius.com:5000/changepassword/${userID}`, { currentPassword, newPassword })
    let promiseB = promiseA.then(({ data }) => {
      ...
      axios
        .post(`http://exporagenius.com:5000/client/login`, { email: email, password: newPassword })
        .then(({ data }) => {
...
export const updateInfo = (userData) => {
  return (dispatch) => {
    axios
      .post(`http://exporagenius.com:5000/client/updateinfo/${userData._id}`, userData)
      .then(({ data }) => {
        //console.log(data);
...
export const loadUser = (tok) => {
  return (dispatch, getState) => {
    ...
    if (token) {
      ...
      axios
        .get(`http://exporagenius.com:5000/client/login`,
          {
            headers: {
