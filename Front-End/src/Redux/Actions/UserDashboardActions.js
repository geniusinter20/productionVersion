...
export const fetchuserpracticetests = (IDs, clientID) => {
    return (dispatch) => {
        IDs.forEach(element => {
            let promiseA = axios.get(`https://exporagenius:5000/practicetests/${element}`)
...
const fetchuserpracticetestExams = (IDs, testID, clientID) => {
    ...
    return (dispatch) => {
        IDs.forEach(element => {
            ...
            axios.
            get(`https://exporagenius:5000/activityexamstate/exam/${element}/${testID}/${clientID}`)
            .then(({data})=>{
            ...
            axios
                .get(`https://exporagenius:5000/exams/${element}`)
                .then(({ data }) => {
...
export const fetchUserExamInfo = (ID) => {
    ...
    return (dispatch) => {
        ...
        let promiseA = axios.get(`https://exporagenius:5000/exams/${ID}`)
        let promiseB = promiseA.then(({ data }) => {
            setTimeout(() => {
                dispatch({
...
const fetchExamQuestions = (IDs) => {
    ...
    return (dispatch) => {
        IDs.forEach(element => {
            axios
                .get(`https://exporagenius:5000/question/${element}`)
                .then(({ data }) => {
...
export const startExam = (activity) => {
    ...
    return (dispatch) => {
        axios.post("https://exporagenius:5000/activityexamstate/add", activity)
            .then(res => {
...
export const fetchActivity = (examID, testID,clientID) => {
    ...
    return (dispatch) => {
        axios.get(`https://exporagenius:5000/activityexamstate/exam/${examID}/${testID}/${clientID}`)
            .then(res => {
                //console.log("res:", res.data)
                 dispatch({
