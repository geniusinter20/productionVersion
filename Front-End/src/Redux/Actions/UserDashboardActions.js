...
export const fetchuserpracticetests = (IDs, clientID) => {
    return (dispatch) => {
        IDs.forEach(element => {
            let promiseA = axios.get(`http://exporagenius.com:5000/practicetests/${element}`)
            ...
            return Promise.all([promiseA, promiseB]).then(function ([responseA, responseB]) {
                promiseB.then(() => {
                    ...
                    setTimeout(() => {
                        dispatch(fetchuserpracticetestExams([...responseA.data[0].testExamsIDs], responseA.data[0]._id, clientID))
                    }, 500)
                })
            });
        });
...
const fetchuserpracticetestExams = (IDs, testID, clientID) => {
    ...
    return (dispatch) => {
        IDs.forEach(element => {
            ...
            axios.
            get(`http://exporagenius.com:5000/activityexamstate/exam/${element}/${testID}/${clientID}`)
            .then(({data})=>{
            ...
            axios
                .get(`http://exporagenius.com:5000/exams/${element}`)
                .then(({ data }) => {
...
export const fetchUserExamInfo = (ID) => {
    ...
    return (dispatch) => {
        ...
        let promiseA = axios.get(`http://exporagenius.com:5000/exams/${ID}`)
        let promiseB = promiseA.then(({ data }) => {
            setTimeout(() => {
                dispatch({
...
const fetchExamQuestions = (IDs) => {
    ...
    return (dispatch) => {
        IDs.forEach(element => {
            axios
                .get(`http://exporagenius.com:5000/question/${element}`)
                .then(({ data }) => {
...
export const startExam = (activity) => {
    ...
    return (dispatch) => {
        axios.post("http://exporagenius.com:5000/activityexamstate/add", activity)
            .then(res => {
...
export const fetchActivity = (examID, testID,clientID) => {
    ...
    return (dispatch) => {
        axios.get(`http://exporagenius.com:5000/activityexamstate/exam/${examID}/${testID}/${clientID}`)
            .then(res => {
                //console.log("res:", res.data)
                 dispatch({
